...
<input type="hidden" name="_token" value="{{ csrf_token() }}" class="token-container">
                            ...
                            <tr data-item-row="{{ $item->id }}" data-unit-price="{{ $unit_price }}" data-discounted-price="{{ $discounted_price }}">
                                <td>
                                    <div class="flex items-center">
                                        ...
                                        <div class="cart-item-details">
                                            ...
                                            <div class="cart-item-variant">{{ Str::limit($item->product->description, 60) }}</div>
                                            @if($promotion)
                                ...
                                <td>
                                    <div class="cart-price">
                                        @if($discount > 0)
                                            <span class="cart-price-original">Rp {{ number_format($unit_price, 0, ',', '.') }}</span>
                                        @endif
                                        <span class="cart-price-discounted">Rp {{ number_format($discounted_price, 0, ',', '.') }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="cart-quantity" data-item-id="{{ $item->id }}">
                                        ...
                                        <input type="number" min="1" max="{{ $item->product->stock }}" value="{{ $qty }}" class="cart-quantity-input" data-item-id="{{ $item->id }}">
                                ...
                                <td>
                                    <div class="cart-price item-total" data-item-id="{{ $item->id }}">Rp {{ number_format($item_total, 0, ',', '.') }}</div>
                                </td>
                                <!-- Untuk desktop view -->
                                <td>
                                    <button type="button" class="cart-remove" data-item-id="{{ $item->id }}" title="Hapus">
                ...
                <div class="mobile-cart">
                    @foreach($cartItems as $item)
                        @php
                            ...
                            $item_total = $discounted_price * $qty;
                        @endphp
                        <div class="mobile-cart-item" data-item-row="{{ $item->id }}" data-unit-price="{{ $unit_price }}" data-discounted-price="{{ $discounted_price }}">
                            <div class="mobile-cart-item-top">
                                <img src="{{ asset($item->product->image_url ?? 'images/no-image.png') }}" alt="{{ $item->product->name }}" class="mobile-cart-item-image">
                                <div class="mobile-cart-item-details">
                                    <div class="mobile-cart-item-name">{{ $item->product->name }}</div>
                                    @if($discount > 0)
                                        <div class="text-sm text-gray-400 line-through">Rp {{ number_format($unit_price, 0, ',', '.') }}</div>
                                    @endif
                                    <div class="mobile-cart-item-price">Rp {{ number_format($discounted_price, 0, ',', '.') }}</div>
                                    @if($promotion)
                                        <div class="mt-2 promo-badge">
                                            <i class="icon" data-feather="tag"></i>
                                            <span>{{ $promotion->promo_code }}</span>
                            ...
                            <div class="mobile-cart-item-bottom">
                                ...
                                <div class="flex items-center gap-4">
                                    <div class="font-semibold mobile-item-total" data-item-id="{{ $item->id }}">Rp {{ number_format($item_total, 0, ',', '.') }}</div>
                                    <!-- Untuk mobile view -->
                                    <button type="button" class="cart-remove" data-item-id="{{ $item->id }}" title="Hapus">
                                        <i data-feather="trash-2"></i>
                                    </button>
                ...
                <div class="cart-panel">
                    ...
                    <div class="cart-panel-body">
                        <div class="cart-summary-row">
                            <span>Subtotal</span>
                            <span id="cart-subtotal">Rp {{ number_format($grand_total + $total_discount, 0, ',', '.') }}</span>
                        </div>
                        <div class="cart-summary-row" id="cart-discount-row" style="{{ $total_discount > 0 ? '' : 'display: none;' }}">
                            <span>Diskon</span>
