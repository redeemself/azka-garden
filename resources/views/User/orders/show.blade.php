    ...
    @if($order)
      ...
      <div class="order-detail-container">
        <div class="flex items-center mb-4">
          ...
          @if($expiredAt && ($statusVal === 'WAITING_PAYMENT' || $statusVal === 'PENDING'))
            <span class="ml-8 text-sm" style="color:#f59e42;">
              Batas pembayaran: <span id="expired-timer">{{ $expiredAt->format('d M Y H:i') }}</span>
            </span>
          @elseif(strtoupper($statusVal) === 'EXPIRED' && isset($order->payment) && isset($order->payment->expired_at))
            <span class="ml-8 text-sm" style="color:#ee5757;">
              Kadaluarsa: {{ \Carbon\Carbon::parse($order->payment->expired_at)->format('d M Y H:i') }}
            </span>
          @endif
        </div>
        <div class="mb-4">
          <span class="order-detail-label">Kode Order:</span>
          <span class="font-mono order-detail-value">{{ $order->order_code }}</span>
        </div>
        <div class="mb-4">
          <span class="order-detail-label">Tanggal Order:</span>
          <span class="order-detail-value">{{ $orderDate->format('d M Y H:i') }}</span>
        </div>
        <div class="mb-4">
          <span class="order-detail-label">Catatan:</span>
          <span class="order-detail-value">{{ $order->note ?? '-' }}</span>
        </div>
        ...
        <div class="order-detail-product-list">
          @foreach($order->details as $detail)
            <div class="order-detail-product-item">
              ...
              @endphp
              <img src="{{ $img }}" alt="{{ $detail->product->name }}" class="order-detail-product-img" />
              <div class="order-detail-product-info">
                <span class="order-detail-product-title">{{ $detail->product->name }}</span>
                <span class="order-detail-product-qty">x{{ $detail->quantity }}</span>
                <span class="order-detail-product-subtotal">
                  @if($hasDiscount)
                    ...
                    <span style="margin-left:.4em;color:#ee5757;font-weight:700;">Rp{{ number_format($detail->subtotal, 0, ',', '.') }}</span>
                  @else
        ...
        <div class="order-detail-summary">
          <div class="order-detail-summary-row">
            ...
            @if($hargaCoret > $totalProduk)
              <span>
                <span class="price-strikethrough">Rp{{ number_format($hargaCoret, 0, ',', '.') }}</span>
                <span style="margin-left:.5em;color:#ee5757;font-weight:700;">Rp{{ number_format($totalProduk, 0, ',', '.') }}</span>
              </span>
            @else
              <span class="order-detail-summary-value">Rp{{ number_format($totalProduk, 0, ',', '.') }}</span>
            @endif
          </div>
          @if($diskon > 0)
          <div class="order-detail-summary-row">
            ...
            <span class="order-detail-summary-discount">-Rp{{ number_format($diskon, 0, ',', '.') }}</span>
          </div>
          @endif
          @if($tax > 0)
          <div class="order-detail-summary-row">
            <span class="order-detail-summary-label">Pajak (PPN 11%)</span>
            <span class="order-detail-summary-tax">+Rp{{ number_format($tax, 0, ',', '.') }}</span>
          </div>
          @endif
          <div class="order-detail-summary-row">
            <span class="order-detail-summary-label">Ongkos Kirim</span>
            <span class="order-detail-summary-value">+Rp{{ number_format($ongkir, 0, ',', '.') }}</span>
          </div>
          <div class="order-detail-summary-row" style="margin-top:1em;">
            ...
            <span class="order-detail-summary-total">
              @if($isCanceledOrExpired)
                <span class="price-strikethrough">Rp{{ number_format($grandTotal, 0, ',', '.') }}</span>
              @else
        ...
        @if($order->shipping)
          <div class="order-detail-shipping">
            ...
            <span class="block">Resi: {{ $order->shipping->tracking_number ?? '-' }}</span>
            <span class="block" style="color:#5f8b6a;">Ongkir: Rp{{ number_format($order->shipping->shipping_cost ?? 0, 0, ',', '.') }}</span>
        ...
        @if($order->payment)
          <div class="order-detail-payment">
            <span class="order-detail-payment-method">{{ $order->payment->method->name ?? '-' }}</span>
            <span class="order-detail-payment-label">Status:
              <span class="{{ $statusClass }}">{{ $paymentStatusText }}</span>
            </span>
            <span class="order-detail-payment-label">Nominal: 
              @if($isCanceledOrExpired)
                <span class="price-strikethrough">Rp{{ number_format($grandTotal, 0, ',', '.') }}</span>
              @else
                <span style="color:#295f3a;font-weight:bold;">Rp{{ number_format($grandTotal, 0, ',', '.') }}</span>
              @endif
            </span>
            @if($diskon > 0 && $promoCode)
              <span class="order-detail-payment-label" style="color:#1a7f73;">
                Diskon aktif: <b>{{ $promoCode }}</b> (-Rp{{ number_format($diskon, 0, ',', '.') }})
              </span>
            @endif
            @if($expiredAt && ($statusVal === 'WAITING_PAYMENT' || $statusVal === 'PENDING'))
              <span class="order-detail-payment-label" style="color:#f59e42;">
                Batas pembayaran: <span id="expired-timer-2">{{ $expiredAt->format('d M Y H:i') }}</span>
              </span>
            @elseif(strtoupper($statusVal) === 'EXPIRED' && isset($order->payment) && isset($order->payment->expired_at))
        ...
        <a href="{{ route('products.index') }}" class="order-detail-back-btn">
            Kembali ke Produk
        </a>
        <a href="{{ route('user.orders.index') }}" class="order-detail-back-btn" style="background:#5f8b6a;">
            Kembali ke Daftar Pesanan
        </a>
        @if(!$isCanceledOrExpired && isset($order->canCancel) && $order->canCancel)
      ...
      @if($expiredAt && ($statusVal === 'WAITING_PAYMENT' || $statusVal === 'PENDING'))
      <script>
        var secondsLeft = {{ max($secondsLeft, 0) }};
