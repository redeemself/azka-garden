...
<div class="confirm-container">
    ...
    <div class="order-card">
        ...
        <div class="info-box">
            ...
            <div><strong>Kota:</strong> {{ $selectedAddress->city }}, <strong>Kode Pos:</strong> {{ $selectedAddress->zip_code }}</div>
    ...
    @if(isset($cartItems) && $cartItems->count())
        <div class="order-card">
                ...
                @foreach ($cartItems as $item)
                    ...
                    <li class="product-item">
                        ...
                        <div class="product-price">
                            @if($discount > 0)
                                <span class="price-strikethrough">Rp{{ number_format($unit_price * $qty, 0, ',', '.') }}</span>
            ...
            <div class="info-block">
                <div><strong>Alamat Pengiriman:</strong> {{ $selectedAddress ? $selectedAddress->full_address : 'Belum dipilih' }}</div>
                ...
                <div><strong>Metode Pembayaran:</strong> {{ $paymentMethodLabels[$payment_method] ?? $payment_method }}</div>
            </div>
        </div>
    ...
    @elseif(isset($order) && isset($order->details) && count($order->details))
        ...
        <div class="order-card">
            ...
            <ul class="product-list">
                @foreach ($order->details as $detail)
                    ...
                    <li class="product-item">
                        <div class="product-content">
                            <img src="{{ asset($image_url) }}" alt="{{ $detail->product->name ?? 'Produk' }}" class="product-image">
                            <div class="product-details">
                        ...
                        <div class="product-price">
                            @if($hasDiscount)
                                <span class="price-strikethrough">Rp{{ number_format($detail->product->price * $detail->quantity, 0, ',', '.') }}</span>
                ...
                <li class="summary-item">
                    ...
                    <span class="summary-value">
                        @if($original_price_total > $subtotal)
                            <span class="price-strikethrough">Rp{{ number_format($original_price_total, 0, ',', '.') }}</span>
        ...
        <div class="action-buttons">
            @if(!$isCanceledOrExpired && ($statusVal === 'WAITING_PAYMENT' || $statusVal === 'PENDING'))
                ...
                <form id="payForm" action="{{ route('user.orders.pay', $order->id) }}" method="POST" style="margin:0; flex: 1;">
                    @csrf
                    <button type="submit" class="button button-primary">
                        Konfirmasi Pembayaran
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
    ...
    @else
        <div class="empty-message">
            ...
            <a href="{{ route('user.cart.index') }}" class="button button-primary" id="back-to-cart" style="margin-left: 0; width: 200px; margin: 0 auto;">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                    <circle cx="9" cy="21" r="1"></circle>
                    <circle cx="20" cy="21" r="1"></circle>
                    <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path>
