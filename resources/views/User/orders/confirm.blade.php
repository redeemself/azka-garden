...
@section('content')
    @php
        ...
        $currentDateTime = '2025-07-30 05:04:16';
        ...
        $shippingMethodDescriptions = [
            'KURIR_TOKO' =>
                'Pengiriman langsung dari toko Azka Garden. Ongkir flat sesuai jarak: (<5km) Rp10.000, (5-10km) Rp15.000, (>10km) Rp20.000',
        ...
        $shippingCostsFromDB = [
            'JNT' => 14000.0, // Pastikan ini 14000, bukan 25000
            'GOSEND' => 25000.0,
            'JNE' => 12000.0,
            'SICEPAT' => 15000.0,
            'KURIR_TOKO' => 15000.0, // default
            'AMBIL_SENDIRI' => 0.0,
        ];
        ...
        if (!isset($shippingCostsFromDB['KURIR_TOKO_LESS_THAN_5KM'])) {
            $shippingCostsFromDB['KURIR_TOKO_LESS_THAN_5KM'] = 10000.0;
        }
        if (!isset($shippingCostsFromDB['KURIR_TOKO_5_TO_10KM'])) {
            $shippingCostsFromDB['KURIR_TOKO_5_TO_10KM'] = 15000.0;
        }
        if (!isset($shippingCostsFromDB['KURIR_TOKO_MORE_THAN_10KM'])) {
            $shippingCostsFromDB['KURIR_TOKO_MORE_THAN_10KM'] = 20000.0;
        }

        // Try to get shipping costs from database
        try {
            $shippingData = DB::table('shippings')->select('courier', 'service', 'shipping_cost')->get();
...
if (isset($cartItems) && $cartItems->count()) {
    ...
    foreach ($order->details as $detail) {
        ...
        $hargaCoret += isset($detail->product->price)
    ...
    <div class="confirm-container">
        ...
        <div class="order-card">
            ...
            <div class="info-block">
                <div><strong>Nama Pengirim:</strong> {{ $toko_name }}</div>
                <div><strong>Alamat Pengirim:</strong> {{ $toko_address }}</div>
            </div>
        </div>
        ...
        <div class="order-card">
            ...
            @if ($selectedAddress)
                <div class="info-box">
                    ...
                    <div><strong>Label:</strong> {{ $selectedAddress->label ?? 'Alamat Utama' }}</div>
                    <div><strong>Penerima:</strong> {{ $selectedAddress->recipient }}</div>
                    <div><strong>Alamat:</strong> {{ $selectedAddress->full_address }}</div>
                    <div><strong>Kota:</strong> {{ $selectedAddress->city }}, <strong>Kode Pos:</strong>
                        {{ $selectedAddress->zip_code }}</div>
                    <div><strong>No HP:</strong> {{ $selectedAddress->phone_number }}</div>
                </div>
            @else
        ...
        <div class="order-card">
            ...
            <div class="info-block">
                <div><strong>{{ $shipping_method_display }}</strong></div>
                <div>{{ $shippingMethodDescriptions[$shipping_method] ?? 'Pengiriman standar' }}</div>
            </div>

            @if ($shipping_method == 'KURIR_TOKO')
                <div class="shipping-method-highlight">
                    @if ($shippingCost == 10000.0)
                    Kurir Toko (Internal) | Jarak < 5km | Ongkir: Rp10.000 @elseif($shippingCost == 15000.0) Kurir
                        Toko (Internal) | Jarak 5-10km | Ongkir: Rp15.000 @elseif($shippingCost == 20000.0) Kurir
                            Toko (Internal) | Jarak> 10km | Ongkir: Rp20.000
                        @else
            ...
            @else
                <div class="shipping-method-highlight">
                    {{ $shipping_method_display }} | Ongkir: Rp{{ number_format($shippingCost, 0, ',', '.') }}
                </div>
            @endif
        </div>
        ...
        @if (isset($cartItems) && $cartItems->count())
            <div class="order-card">
                ...
                <ul class="product-list">
                    @foreach ($cartItems as $item)
                        @php
                            ...
                            $item_promo = $item->promo_code ?? $promo_code;
                            $promotion = $item_promo
                                ? \App\Models\Promotion::where('promo_code', $item_promo)->first()
                                : null;
                            $discount = 0;
                            $unit_price = $product->price ?? 0;
                            $qty = $item->quantity ?? 0;
                        ...
                        <li class="product-item">
                            <div class="product-content">
                                <img src="{{ asset($image_url) }}" alt="{{ $product->name }}" class="product-image">
                                <div class="product-details">
                                    <div class="product-name">{{ $product->name }}</div>
                                    <div class="product-quantity">x{{ $qty }}</div>
                                    @if ($item_promo && $promotion)
                            ...
                            <div class="product-price">
                                @if ($discount > 0)
                                    <span
                                        class="price-strikethrough">Rp{{ number_format($unit_price * $qty, 0, ',', '.') }}</span>
                ...
                <div class="info-block">
                    <div><strong>Alamat Pengiriman:</strong>
                        {{ $selectedAddress ? $selectedAddress->full_address : 'Belum dipilih' }}</div>
                    <div><strong>Metode Pengiriman:</strong> {{ $shipping_method_display }}
                        @if ($shipping_method == 'AMBIL_SENDIRI')
                            (Gratis)
                        @else
                    ...
                    <div><strong>Metode Pembayaran:</strong> {{ $paymentMethodLabels[$payment_method] ?? $payment_method }}
                    </div>
                </div>
            </div>
            ...
            <div class="order-card">
                ...
                <ul class="summary-list">
                    <li class="summary-item">
                        ...
                        <span class="summary-value">Rp{{ number_format($original_price_total, 0, ',', '.') }}</span>
                    </li>
                    @if ($total_discount > 0)
                        <li class="summary-item discount">
                            <span class="summary-label">Diskon</span>
                            <span class="summary-value">-Rp{{ number_format($total_discount, 0, ',', '.') }}</span>
                        </li>
                    @endif
                    @if ($handlingFee > 0)
                        <li class="summary-item">
                            <span class="summary-label">Biaya Penanganan</span>
                            <span class="summary-value">Rp{{ number_format($handlingFee, 0, ',', '.') }}</span>
                        </li>
                    @endif
                    <li class="summary-item">
                    ...
                    @if ($paymentFee > 0)
                        <li class="summary-item">
                            ...
                            <span class="summary-value">Rp{{ number_format($paymentFee, 0, ',', '.') }}</span>
                        </li>
                    @endif
                    <li class="summary-item tax">
                        <span class="summary-label">PPN 11%</span>
                        <span class="summary-value">Rp{{ number_format($taxAmount, 0, ',', '.') }}</span>
                    </li>
                    <li class="summary-item summary-total">
                        <span class="summary-label">Total Pembayaran</span>
                        <span class="summary-value">Rp{{ number_format($totalWithTax, 0, ',', '.') }}</span>
                    </li>
            ...
            <div class="action-buttons">
                <a href="{{ route('user.cart.index') }}" class="button button-secondary" id="back-to-cart">
                ...
                <form id="createOrderForm" action="{{ route('user.orders.create') }}" method="POST"
                    style="margin:0; flex: 1;">
                    @csrf
                    <input type="hidden" name="shipping_method" value="{{ $shipping_method }}">
                    <input type="hidden" name="shipping_service"
                        value="{{ $shippingServiceNames[$shipping_method] ?? '-' }}">
                    @if ($selectedAddress)
        ...
        @elseif(isset($order) && isset($order->details) && count($order->details))
            @php
                ...
                $statusVal = is_object($order->status)
                    ? $order->status->value
                    : (is_array($order->status)
                        ? $order->status['value'] ?? ($order->status[0] ?? $order->status)
                        : $order->status);
            ...
            <div class="order-card">
                ...
                <ul class="product-list">
                    @foreach ($order->details as $detail)
                        @php
                            $hasDiscount =
                                isset($detail->product->price) && $detail->product->price > $detail->subtotal;
                            $image_url = $detail->product->image_url ?? 'images/no-image.png';
                        @endphp
                        <li class="product-item">
                            <div class="product-content">
                                <img src="{{ asset($image_url) }}" alt="{{ $detail->product->name ?? 'Produk' }}"
                                    class="product-image">
                                <div class="product-details">
                                    <div class="product-name">{{ $detail->product->name ?? 'Produk' }}</div>
                            ...
                            <div class="product-price">
                                @if ($hasDiscount)
                                    <span
                                        class="price-strikethrough">Rp{{ number_format($detail->product->price * $detail->quantity, 0, ',', '.') }}</span>
                ...
                <div class="info-block">
                    <div><strong>Alamat Pengiriman:</strong>
                        @if ($order->shipping_address)
                            {{ is_string($order->shipping_address) ? $order->shipping_address : json_encode($order->shipping_address) }}
                    ...
                    <div><strong>Metode Pengiriman:</strong> {{ $orderShippingMethod }}
                        @if ($order->shipping_method == 'AMBIL_SENDIRI')
                            (Gratis)
                        @else
                            (Rp{{ number_format($shippingCost, 0, ',', '.') }})
                    ...
                    <div><strong>Metode Pembayaran:</strong> {{ $displayPaymentMethod }}</div>
                    <div>
                        <strong>Status:</strong>
                        @if ($statusVal == 'WAITING_PAYMENT' || $statusVal == 'PENDING')
                        ...
                        @else
                            <span class="status-badge">{{ ucfirst(strtolower($statusVal)) }}</span>
                        @endif
                    </div>
                </div>
            </div>
                ...
                <ul class="summary-list">
                    <li class="summary-item">
                        ...
                        <span class="summary-value">
                            @if ($original_price_total > $subtotal)
                                <span
                                    class="price-strikethrough">Rp{{ number_format($original_price_total, 0, ',', '.') }}</span>
                    ...
                    <li class="summary-item summary-total">
                        ...
                        <span class="summary-value">
                            @if ($isCanceledOrExpired)
                                <span class="price-strikethrough">Rp{{ number_format($totalWithTax, 0, ',', '.') }}</span>
                            @else
            ...
            <div class="action-buttons">
                @if (!$isCanceledOrExpired && ($statusVal === 'WAITING_PAYMENT' || $statusVal === 'PENDING'))
                    <a href="{{ route('user.cart.index') }}" class="button button-secondary" id="back-to-cart">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                            viewBox="0 0 24 24">
                    ...
                    <form id="payForm" action="{{ route('user.orders.pay', $order->id) }}" method="POST"
                        style="margin:0; flex: 1;">
                        @csrf
                        <button type="submit" class="button button-primary">
                            Konfirmasi Pembayaran
                ...
                @else
                    <a href="{{ route('user.orders.index') }}" class="button button-primary" style="margin-left: 0;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                            viewBox="0 0 24 24">
                            <path d="M19 12H5M12 19l-7-7 7-7"></path>
        ...
        @else
            <div class="empty-message">
                ...
                <a href="{{ route('user.cart.index') }}" class="button button-primary" id="back-to-cart"
                    style="margin-left: 0; width: 200px; margin: 0 auto;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        viewBox="0 0 24 24">
                        <circle cx="9" cy="21" r="1"></circle>
