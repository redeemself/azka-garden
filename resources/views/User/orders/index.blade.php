...
@section('content')
    @php
        ...
        // Data waktu dan user dari halaman pembayaran (diperbarui sesuai permintaan)
        $currentDateTime = '2025-07-31 10:38:53';
        $currentUserLogin = 'DenuJanuari';
        ...
        $getStatusValue = function ($status) {
            ...
            if (is_array($status)) {
                return $status['value'] ?? ($status[0] ?? $status);
            }
            return $status;
        };
        ...
        $processingOrShippedOrders = $activeOrders->filter(function ($order) use (
            $getStatusValue,
        ...
        $confirmedOrders = $activeOrders->filter(function ($order) use (
            $getStatusValue,
            $isPending,
            $isProcessingOrShipped,
        ) {
        ...
        $getShippingCost = function ($order) use ($fixedShippingCosts) {
            ...
            switch ($shipping_method) {
                case 'KURIR_TOKO':
                    ...
                    $shippingCost =
                        $shippingCostMap['KURIR_TOKO'] ??
                        ($shippingCostMap['KURIR TOKO'] ?? $fixedShippingCosts['KURIR_TOKO']);
        ...
        // Updated calculation of total with tax using shipping cost function and checkout discount logic
        $getTotalWithTax = function ($order) use ($getShippingCost) {
            ...
            // Get promo info from order if available
            $promo_code = $order->promo_code ?? '';
            $promo_type = $order->promo_type ?? '';
            $promo_discount = $order->promo_discount ?? 0;

            // Calculate discount based on checkout.blade.php logic
            $discount = 0;
            if ($promo_type === 'percent') {
                $discount = $subtotal * ($promo_discount / 100);
            } elseif ($promo_discount > 0) {
                $discount = $promo_discount;
            } else {
                // Use existing discount if no promo info
            ...
            // Calculate subtotal after discount (consistent with checkout.blade.php)
            ...
            $total = $subtotalAfterDiscount + $tax + $shipping + $handling + $payment;
    ...
    <style>
        ...
        .expired-table-container {
            ...
            width: 100%;
            max-width: 100%;
            /* Perlebar container menjadi 100% */
            margin-left: auto;
            margin-right: auto;
            padding-bottom: 1em;
            box-sizing: border-box;
            overflow-x: auto;
            /* Tambahkan overflow untuk tabel lebar */
        }
        ...
        .expired-orders-table th,
        .expired-orders-table td {
            padding: 1.15rem 0.5em;
            /* Kurangi padding horizontal */
            vertical-align: top;
        ...
        .canceled-orders-table th:not(:last-child),
        .canceled-orders-table td:not(:last-child),
        .expired-orders-table th:not(:last-child),
        .expired-orders-table td:not(:last-child) {
            padding-right: 1em;
            /* Kurangi padding kanan */
        }

        .orders-table thead th,
        .canceled-orders-table thead th,
        .expired-orders-table thead th {
            background: #e8f4ea;
            color: #295f3a;
            font-weight: 700;
            font-size: 1.07rem;
        ...
        .orders-table tbody tr.waiting-row {
            background: #f6f9f6;
            box-shadow: 0 0 0 2px #e8f4ea;
        }

        .orders-table tbody tr:hover,
        .canceled-orders-table tbody tr:hover,
        .expired-orders-table tbody tr:hover {
            background: #f0f5f2;
            box-shadow: 0 2px 12px rgba(80, 120, 90, 0.07);
            transform: scale(1.002);
            /* Kurangi skalanya lagi */
        }
        ...
        .status-badge {
            ...
            border: none;
            white-space: nowrap;
            /* Mencegah pemisahan teks */
            margin-bottom: 0.5em;
            /* Tambahkan margin di bawah badge status */
        }
        ...
        .action-btns {
            ...
            gap: 6px;
            /* Kurangi gap antar tombol */
            margin-top: 0.3em;
            align-items: stretch;
            width: 100%;
            /* Memastikan lebar tombol-tombol konsisten */
        }
        ...
        .btn-detail:hover {
            background: #d7f5e3;
            box-shadow: 0 2px 18px #bde2c533;
        }
        ...
        .canceled-warning {
            max-width: 100%;
            /* Perlebar sesuai container utama */
        ...
        @media (max-width: 640px) {
            ...
            .expired-orders-table,
            .orders-table thead,
            .canceled-orders-table thead,
            .expired-orders-table thead,
            .orders-table tr,
            .canceled-orders-table tr,
            .expired-orders-table tr {
            ...
            .orders-table tbody,
            .canceled-orders-table tbody,
            .expired-orders-table tbody {
                display: block !important;
                width: 100% !important;
            }
            ...
            .canceled-orders-table td:before,
            .expired-orders-table td:before {
        ...
        .product-list {
            ...
            gap: 0.8rem;
            max-width: 250px;
            /* Mengurangi lebar maksimal untuk kolom produk */
            padding-right: 8px;
            /* Kurangi padding kanan */
        ...
        /* Kecilkan kolom kode order */
        .column-date {
            width: 8%;
        }

        /* Kecilkan kolom tanggal */
        .column-status {
            width: 12%;
        }
        ...
        /* Perlebar kolom pengiriman */
        .column-payment {
            width: 18%;
        }

        /* Perlebar kolom pembayaran */
        .column-action {
            width: 10%;
        }
        ...
        .column-status::after {
            ...
            top: 15%;
            right: 8px;
            /* Sesuaikan posisi garis pembatas */
            height: 70%;
            width: 1px;
            background-color: #eaf8f0;
        }
        ...
        .column-price {
            ...
            /* Kurangi jarak di kiri total harga */
        }
        ...
        /* Style untuk badge diskon - baru ditambahkan */
        .discount-badge {
            display: inline-flex;
            align-items: center;
            padding: 0.25rem 0.75rem;
        ...
        /* Style untuk menampilkan diskon */
        .discount-info {
            margin-top: 0.4rem;
    ...
    <div x-data="{ loading: true }" x-init="setTimeout(() => loading = false, 400)">
        ...
        <div class="px-2 mx-auto sm:px-5 max-w-7xl" x-show="!loading" x-transition.opacity>
            <h1 class="mb-8 text-3xl font-extrabold tracking-tight text-center" style="color:#295f3a;">Daftar Pesanan Anda
            </h1>
                                ...
                                @foreach ($pendingOrders as $order)
                                    @php
                                        ...
                                        $shippingMethod = $getShippingMethodName(
                                            $order->shipping_method ?? 'KURIR_TOKO',
                                        );
                                        ...
                                        // Promo details
                                        $hasPromo = !empty($order->promo_code) || !empty($order->promo_type);
                                        $promoDisplay = '';
                                        if ($hasPromo) {
                                            if ($order->promo_type === 'percent') {
                                                $promoDisplay = "Diskon {$order->promo_discount}%";
                                            } elseif (!empty($order->promo_discount)) {
                                                $promoDisplay =
                                                    'Diskon Rp' . number_format($order->promo_discount, 0, ',', '.');
                                            }
                                        }
                                    @endphp
                                    <tr class="waiting-row">
                                        <td data-label="Kode Order" class="column-order-code"><span
                                                class="font-mono font-bold"
                                                style="color:#295f3a;">{{ $order->order_code }}</span></td>
                                        <td data-label="Tanggal" class="column-date">
                                            <span
                                                class="block">{{ \Carbon\Carbon::parse($order->order_date ?? $order->created_at)->format('d M Y') }}</span>
                                            <span class="block text-xs"
                                                style="color:#888e92;">{{ \Carbon\Carbon::parse($order->order_date ?? $order->created_at)->format('H:i') }}</span>
                                        </td>
                                        ...
                                        <td data-label="Total Harga" class="column-price">
                                            <span
                                                class="total-price">Rp{{ number_format($totalWithTax, 0, ',', '.') }}</span>
                                            @if ($hasPromo && !empty($promoDisplay))
                                                <div class="discount-info">{{ $promoDisplay }}</div>
                                            @endif
                                        </td>
                                        <td data-label="Produk" class="column-product">
                                            <div class="product-list">
                                                @foreach ($order->details as $detail)
                                                    @php
                                                        $img =
                                                            isset($detail->product->images) &&
                                                            $detail->product->images->count() > 0
                                                    ...
                                                    <div class="product-item">
                                                        ...
                                                        <div class="product-info">
                                                            ...
                                                            <span
                                                                class="product-subtotal">Rp{{ number_format($detail->subtotal, 0, ',', '.') }}</span>
                                                        </div>
                                                    </div>
                                                @endforeach
                                            </div>
                                        ...
                                        <td data-label="Pengiriman" class="column-shipping">
                                            @if ($order->shipping)
                                                <div class="shipping-info">
                                                    <span
                                                        class="font-semibold">{{ $order->shipping->courier ?? $shippingMethod }}</span>
                                                    @if ($isFreeShipping)
                                                    ...
                                                    <span class="block">Resi: <span
                                                            class="font-mono">{{ $order->shipping->tracking_number ?? '-' }}</span></span>
                                                    <span class="block" style="color:#5f8b6a;">Status:
                                                        {{ $order->shipping->status ?? '-' }}</span>
                                                </div>
                                            @else
                                                <div class="shipping-info">
                                        ...
                                        <td data-label="Pembayaran" class="column-payment">
                                            <div class="payment-info">
                                                <span class="payment-label">Metode:
                                                    {{ $order->payment->method->name ?? $paymentMethod }}</span><br>
                                                <span class="payment-label">Status: Menunggu Pembayaran</span><br>
                                                <span style="font-size:0.97em;">Total:</span>
                                                <span
                                                    class="payment-nominal">Rp{{ number_format($totalWithTax, 0, ',', '.') }}</span>
                                            </div>
                                        </td>
                                        <td data-label="Aksi" class="column-action">
                                            <div class="action-btns">
                                                <a href="{{ route('user.orders.show', $order->id) }}"
                                                    class="action-btn btn-detail">Detail Pesanan</a>
                                                <button type="button" class="action-btn btn-cancel"
                                                    onclick="document.getElementById('cancel-form-{{ $order->id }}').submit()">Batalkan</button>
                                                <button type="button" class="action-btn btn-expire"
                                                    onclick="showExpireConfirm({{ $order->id }})">Jadikan
                                                    Kadaluarsa</button>
                                                <button type="button" class="action-btn btn-complete"
                                                    onclick="document.getElementById('complete-form-{{ $order->id }}').submit()">Selesaikan</button>

                                                <form id="cancel-form-{{ $order->id }}"
                                                    action="{{ route('user.orders.cancel', $order->id) }}" method="POST"
                                                    style="display:none;">
                                                    @csrf @method('PATCH')
                                                </form>
                                                <form id="expire-form-{{ $order->id }}"
                                                    action="{{ route('user.orders.expire', $order->id) }}" method="POST"
                                                    style="display:none;" class="expire-form"
                    ...
                    <span class="icon-processing">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            ...
                            <circle cx="12" cy="12" r="10" stroke="#bbb" stroke-width="2"
                                fill="#f4f4f7" />
                            <path d="M12 7v5" stroke="#bbb" stroke-width="2" stroke-linecap="round" />
                            <circle cx="12" cy="16" r="1.2" fill="#bbb" />
                        </svg>
                    </span>
                    Pesanan Anda sedang <b>diproses</b> atau <b>dikirim</b>. Silakan cek detail pesanan untuk status
                    terbaru.
                </div>
                <div class="orders-table-container">
                            ...
                            <tbody>
                                @foreach ($processingOrShippedOrders as $order)
                                    ...
                                    <tr>
                                        <td data-label="Kode Order" class="column-order-code"><span
                                                class="font-mono font-bold"
                                                style="color:#888;">{{ $order->order_code }}</span></td>
                                        <td data-label="Tanggal" class="column-date">
                                            <span
                                                class="block">{{ \Carbon\Carbon::parse($order->order_date ?? $order->created_at)->format('d M Y') }}</span>
                                            <span class="block text-xs"
                                        ...
                                        <td data-label="Total Harga" class="column-price">
                                            <span class="total-price"
                                                style="color:#888;">Rp{{ number_format($totalWithTax, 0, ',', '.') }}</span>
                                            @if ($hasPromo && !empty($promoDisplay))
                                                <div class="discount-info">{{ $promoDisplay }}</div>
                                            @endif
                                        </td>
                                            ...
                                            <div class="product-list">
                                                @foreach ($order->details as $detail)
                                                    ...
                                                    <div class="product-item">
                                                        ...
                                                        <div class="product-info">
                                                            <span
                                                                class="product-title">{{ $detail->product->name }}</span>
                                                            <span class="product-qty">x{{ $detail->quantity }}</span>
                                                            <span
                                                                class="product-subtotal">Rp{{ number_format($detail->subtotal, 0, ',', '.') }}</span>
                                                        </div>
                                        ...
                                        <td data-label="Pengiriman" class="column-shipping">
                                            @if ($order->shipping)
                                                <div class="shipping-info">
                                                    ...
                                                    <span class="block">Resi: <span
                                                            class="font-mono">{{ $order->shipping->tracking_number ?? '-' }}</span></span>
                                                    <span class="block" style="color:#888;">Status:
                                                        {{ $order->shipping->status ?? $statusText }}</span>
                                                </div>
                                            @else
                                                <div class="shipping-info">
                                                    <span class="font-semibold">{{ $shippingMethod }}</span>
                                        ...
                                        <td data-label="Pengiriman" class="column-shipping">
                                            @if ($order->shipping)
                                                <div class="shipping-info">
                                                    ...
                                                    <span class="block">Resi:
                                                        <span
                                                            class="font-mono">{{ $order->shipping->tracking_number ?? '-' }}</span>
                                                    </span>

                                                    <span
                                                        class="font-mono">{{ $order->shipping->tracking_number ?? '-' }}</span>
                                                    <span class="block" style="color:#5f8b6a;">Status:
                                                        {{ $order->shipping->status ?? 'Selesai' }}</span>
                                                    <span class="block" style="color:#5f8b6a;">Status:
                                                        {{ $order->shipping->status ?? 'Selesai' }}</span>
                                                </div>
                                            @else
                                        ...
                                        <td data-label="Pembayaran" class="column-payment">
                                            <div class="payment-info">
                                                ...
                                                <span
                                                    class="payment-nominal">Rp{{ number_format($totalWithTax, 0, ',', '.') }}</span>
                                                @if ($hasPromo && !empty($promoDisplay))
                                                    <div style="color:#15803d;font-size:0.97em; margin-top:0.1em;">
                                                        {{ $promoDisplay }}
                                                    </div>
                                                @endif
                                            </div>
            ...
            @if ($canceledOrders->count())
                <div class="canceled-warning">
                    <span class="icon-warn"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            style="height:1.4em;width:1.4em;">
                            <circle cx="12" cy="12" r="10" stroke="#ffbb00" stroke-width="2"
                                fill="#fffbe5" />
                            <path d="M12 7v5" stroke="#ffbb00" stroke-width="2" stroke-linecap="round" />
                            <circle cx="12" cy="16" r="1.2" fill="#ffbb00" />
                        </svg></span>
                            ...
                            <tbody>
                                @foreach ($canceledOrders as $order)
                                    ...
                                    <tr>
                                        <td data-label="Kode Order" class="column-order-code"><span
                                                class="font-mono font-bold"
                                                style="color:#888e92;">{{ $order->order_code }}</span></td>
                                        <td data-label="Tanggal" class="column-date">
                                            <span
                                                class="block">{{ \Carbon\Carbon::parse($order->order_date ?? $order->created_at)->format('d M Y') }}</span>
                                            <span class="block text-xs"
                                        ...
                                        <td data-label="Total Harga" class="column-price">
                                            <span
                                                class="total-price price-strikethrough">Rp{{ number_format($totalWithTax, 0, ',', '.') }}</span>
                                            @if ($hasPromo && !empty($promoDisplay))
                                                <div class="discount-info" style="color:#888;">{{ $promoDisplay }}</div>
                                            @endif
                                        </td>
                                        <td data-label="Produk" class="column-product">
                                            <div class="product-list">
                                                    ...
                                                    <div class="product-item">
                                                        ...
                                                        <div class="product-info">
                                                            <span class="product-title"
                                                                style="color:#888e92;">{{ $detail->product->name }}</span>
                                                            <span class="product-qty">x{{ $detail->quantity }}</span>
                                                            <span class="product-subtotal price-strikethrough"
                                                                style="color:#888e92;">
                                            ...
                                            @if ($order->shipping)
                                                <div class="shipping-info">
                                                    <span class="font-semibold"
                                                        style="color:#888e92;">{{ $order->shipping->courier ?? $shippingMethod }}</span>
                                                    @if ($isFreeShipping)
                                                        <span class="free-shipping-badge"
                                                            style="background:#888e92;">BEBAS ONGKIR</span>
                                                    @endif
                                                    <span class="block">{{ $order->shipping->service ?? '-' }}</span>
                                                    <span class="block" style="color:#888e92;">
                                                        ...
                                                        @else
                                                            <span
                                                                class="price-strikethrough">Rp{{ number_format($order->shipping->shipping_cost ?? $shippingCost, 0, ',', '.') }}</span>
                                                        @endif
                                                    </span>
                                                    <span class="block">Resi: <span
                                                            class="font-mono">{{ $order->shipping->tracking_number ?? '-' }}</span></span>
                                                    <span class="block" style="color:#888e92;">Status:
                                                        {{ $order->shipping->status ?? 'Dibatalkan' }}</span>
                                                </div>
                                            @else
                                                <div class="shipping-info">
                                                    <span class="font-semibold"
                                                        style="color:#888e92;">{{ $shippingMethod }}</span>
                                                    @if ($isFreeShipping)
                                                        <span class="free-shipping-badge"
                                                            style="background:#888e92;">BEBAS ONGKIR</span>
                                                    @endif
                                                    <span class="block" style="color:#888e92;">
                                                        ...
                                                        @else
                                                            <span
                                                                class="price-strikethrough">Rp{{ number_format($shippingCost, 0, ',', '.') }}</span>
                                                        @endif
                                                    </span>
                                        ...
                                        <td data-label="Pembayaran" class="column-payment">
                                            <div class="payment-info" style="background:#f8faf8;">
                                                ...
                                                @if ($hasPromo && !empty($promoDisplay))
                                                    <div style="color:#888;font-size:0.97em; margin-top:0.1em;">
                                                        {{ $promoDisplay }}
                                                    </div>
                                                @endif
                                            </div>
                                            ...
                                            @if ($order->shipping)
                                                <div class="shipping-info">
                                                    ...
                                                    <span class="block">Resi: <span
                                                            class="font-mono">{{ $order->shipping->tracking_number ?? '-' }}</span></span>
                                                    <span class="block" style="color:#888e92;">Status:
                                                        {{ $order->shipping->status ?? 'Kadaluarsa' }}</span>
                                                </div>
                                            @else
                                                <div class="shipping-info">
                                                    <span class="font-semibold"
            ...
            @if (
                ...
                <div
                    class="w-full max-w-2xl p-10 mx-auto mt-8 text-center text-gray-500 bg-white border-2 border-green-200 shadow-sm rounded-xl">
            ...
            <div id="expire-confirm-modal"
                style="display:none; position:fixed; inset:0; background:rgba(240,245,248,0.82);z-index:1000;justify-content:center;align-items:center;">
                <div
                    style="background:#fff; border-radius:1.3em; box-shadow:0 6px 40px #4443; padding:2.2em 2.2em 1.5em 2.2em; min-width:320px; max-width:96vw; text-align:center; position:relative;">
                    <span style="position:absolute; top:1em; right:1em; cursor:pointer;" onclick="hideExpireConfirm()">
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="none">
                            <path d="M6 6l13 13M19 6L6 19" stroke="#aaa" stroke-width="2.2" stroke-linecap="round" />
                        </svg>
                    </span>
                    <div style="display:flex;align-items:center;justify-content:center;margin-bottom:1em;">
                        <span style="color:#ffbb00;font-size:1.5em;margin-right:0.65em;"><svg
                    ...
                    <div style="display:flex;justify-content:center;gap:1em;">
                        <button type="button" class="expire-cancel-btn"
                            style="padding:0.6em 1.6em; border-radius:0.7em; border:none; background:#f3f3f6; color:#222; font-weight:600; font-size:1em; margin-right:0.25em;cursor:pointer;"
                            onclick="hideExpireConfirm()">Batal</button>
                        <button type="button" class="expire-ok-btn"
                            style="padding:0.6em 1.6em; border-radius:0.7em; border:none; background:#ffd600; color:#222; font-weight:700; font-size:1em;cursor:pointer;"
                            onclick="submitExpireOrder()">OK</button>
                    </div>
                </div>
            </div>
            <script>
                ...
                const currentDateTime = '2025-07-31 10:43:02';
                const currentUser = 'DenuJanuari';
