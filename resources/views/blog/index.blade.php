...
@section('title', 'Blog - Azka Garden')
...
<section class="py-12 bg-green-50" x-data="{ showAll: false }">
  <div class="max-w-6xl px-4 mx-auto space-y-8 overflow-hidden sm:px-6">
    ...
    {{-- Form Pencarian --}}
    <form action="{{ route('blog.search') }}" method="GET" class="flex w-full max-w-md gap-2 mb-8" role="search" aria-label="Form pencarian artikel">
      <input
        type="text"
        name="q"
        value="{{ old('q', $query ?? '') }}"
      ...
      <select name="source" aria-label="Pilih sumber pencarian" class="min-w-0 px-3 border-t border-b border-green-300 focus:outline-none">
        <option value="web" {{ (isset($source) && $source === 'web') ? 'selected' : '' }}>Web</option>
        <option value="scholar" {{ (isset($source) && $source === 'scholar') ? 'selected' : '' }}>Scholar</option>
      </select>
      <button
        type="submit"
    ...
    <div class="max-w-6xl px-4 mx-auto">
        ...
        <a href="{{ $post['url'] ?? '#' }}" target="_blank" rel="noopener noreferrer"
            ...
            @if(!empty($post['date']))
            <time datetime="{{ \Carbon\Carbon::parse($post['date'])->format('Y-m-d') }}" class="mt-auto text-sm text-green-500">{{ $post['date'] }}</time>
            @endif
        </a>
        @empty
