...
@section('content')
    ...
    <section x-data="{
    ...
    }" x-init="init" class="relative w-full min-h-screen overflow-hidden">
        <div class="absolute inset-0 z-0 pointer-events-none">
            <template x-if="transitioning">
                <div class="absolute inset-0 bg-center bg-cover will-change-transform"
        ...
        {{-- MODAL: Guest = Sarankan Registrasi --}}
        @guest('web')
            <div x-data="{ show: true }" x-show="show" x-transition
                class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" style="backdrop-filter: blur(3px);">
                <div
                    class="relative flex flex-col items-center w-full max-w-xs p-6 bg-white border border-blue-200 shadow-2xl rounded-xl">
                    ...
                    <div class="flex flex-col items-center">
                        ...
                        <div class="mb-5 text-sm text-center text-gray-700">Anda belum login. Daftarkan akun baru untuk
                            pengalaman belanja lebih mudah dan cepat di Azka Garden.</div>
                        ...
                        <button @click="show = false"
                            class="w-full px-4 py-2 mt-2 font-semibold text-center text-gray-800 transition bg-gray-100 border border-gray-200 rounded-lg shadow hover:bg-gray-200">Tutup</button>
                    </div>
                </div>
            </div>
        @endguest

        {{-- MODAL: Sudah Login tapi belum isi alamat --}}
        @auth('web')
            @if (!$hasAddress)
                <div x-data="{ show: true }" x-show="show" x-transition
                    class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" style="backdrop-filter: blur(3px);">
                    <div
                        class="relative flex flex-col items-center w-full max-w-xs p-6 bg-white border border-yellow-200 shadow-2xl rounded-xl">
                        <button @click="show = false"
                            class="absolute px-2 text-xl font-bold text-gray-400 top-2 right-2 hover:text-gray-600"
                            aria-label="Tutup">&times;</button>
                        <div class="flex flex-col items-center">
                            ...
                            <div class="mt-2 text-xs text-gray-600">Setelah alamat terisi, silakan pilih produk yang Anda
                                inginkan dengan baik dan benar.</div>
                            <button @click="show = false"
                                class="w-full px-4 py-2 mt-3 font-semibold text-center text-gray-800 transition bg-gray-100 border border-gray-200 rounded-lg shadow hover:bg-gray-200">Tutup</button>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="container px-3 py-8 mx-auto md:px-8 lg:px-16">
                <div class="flex flex-col items-center px-4 mb-12 text-center">
                    ...
                    <p class="mb-6 text-xl md:text-2xl text-white/80 drop-shadow">
                        Temukan tanaman & layanan lanskap terbaik hanya di <span class="font-bold text-green-200">Azka
                            Garden</span>.
                    </p>
                            ...
                            <div class="flex items-center gap-3">
                                <div class="relative">
                                    ...
                                    <span class="absolute flex w-4 h-4 -top-2 -right-2">
                                        <span
                                            class="absolute inline-flex w-full h-full bg-white rounded-full opacity-75 animate-ping"></span>
                        ...
                        <a href="https://www.tokopedia.com/hendrikfloris" target="_blank" rel="noopener"
                            ...
                            <div class="flex items-center gap-3">
                                <svg class="w-6 h-6" viewBox="0 0 24 24" fill="#42B549">
                                    <path
                                        d="M12 1.25C6.072 1.25 1.25 6.072 1.25 12S6.072 22.75 12 22.75 22.75 17.928 22.75 12 17.928 1.25 12 1.25zm0 1.5c5.109 0 9.25 4.141 9.25 9.25s-4.141 9.25-9.25 9.25S2.75 17.109 2.75 12 6.891 2.75 12 2.75z" />
                                    <path
                                        d="M8.797 15.333c-1.375 0-2.5-1.419-2.5-3.158 0-1.74 1.125-3.158 2.5-3.158s2.5 1.419 2.5 3.158c0 1.74-1.125 3.158-2.5 3.158zm0-4.816c-.517 0-.937.744-.937 1.658s.42 1.658.937 1.658.938-.744.938-1.658-.421-1.658-.938-1.658zm6.406 4.816c-1.375 0-2.5-1.419-2.5-3.158 0-1.74 1.125-3.158 2.5-3.158s2.5 1.419 2.5 3.158c0 1.74-1.125 3.158-2.5 3.158zm0-4.816c-.517 0-.938.744-.938 1.658s.421 1.658.938 1.658.937-.744.937-1.658-.42-1.658-.937-1.658z" />
                                </svg>
                        ...
                        <div class="col-span-1 sm:col-span-2 lg:col-span-3">
                            <template x-for="(step, idx) in steps" :key="idx">
                                <div x-show="current === idx && !stepTransitioning"
                                    ...
                                    <p class="mb-1 text-sm text-center md:text-base text-white/90" x-text="step.desc"></p>
                                    <div class="flex items-center justify-center gap-2 mt-1 mb-1">
                                        <template x-for="(s, i) in steps" :key="'dot-' + i">
                                            <span
                                                :class="{
                                                    'w-4 h-2 rounded-full bg-green-500 shadow transition-all duration-300': i ===
                                                        current,
                                    ...
                                    <div class="flex justify-center gap-2 mt-1">
                                        <button
                                            ...
                                            x-on:click="prevStep" x-bind:disabled="current === 0">
                                            Sebelumnya
                                        </button>
                                        <button
                                            ...
                                            x-on:click="nextStep"
                                            x-text=" current < steps.length - 1 ? 'Paham' : 'Mulai Sekarang' "></button>
                                    </div>
                                </div>
                            </template>
                        </div>
                ...
                <div x-show="showRegister" x-transition:enter="transition ease-out duration-300"
                    ...
                    <div class="w-full max-w-lg p-8 bg-white shadow-xl rounded-xl">
                        ...
                        <a href="{{ route('products.index') }}"
                            class="block px-4 py-2 font-semibold text-center text-green-700 transition rounded hover:underline hover:bg-green-100 bg-green-50">
                            Menuju Halaman Produk
                        </a>
                        <button @click="showRegister = false"
                            class="px-3 py-1 mt-4 text-gray-600 bg-gray-200 rounded hover:bg-gray-300">Tutup</button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-8 mb-12 sm:grid-cols-2 lg:grid-cols-3">
                    @forelse($newestProducts as $product)
                        <div
                            class="flex flex-col overflow-hidden transition bg-white/90 border border-green-200 rounded-2xl shadow-lg hover:shadow-2xl h-full backdrop-blur-lg hover:-translate-y-1 hover:scale-[1.02] duration-300">
                            <img src="{{ asset($product->image_url) }}" alt="{{ $product->name }}"
                                class="object-cover w-full h-40 border-b border-green-100 rounded-t-2xl" loading="lazy">
                            <div class="flex flex-col flex-1 p-6">
                                <h2 class="mb-2 text-2xl font-semibold text-green-800">{{ $product->name }}</h2>
                ...
                <div class="mb-12 text-right">
                    <a href="{{ url('/products') }}"
                        class="px-5 py-2.5 text-white bg-green-700 rounded-xl hover:bg-green-800 shadow font-semibold hover:scale-105 duration-200">Lihat
                        Semua Produk</a>
                </div>
                ...
                <div class="grid grid-cols-1 gap-8 mb-12 sm:grid-cols-2 lg:grid-cols-3">
                    @php
                        $rekomendasiPerluDibeli = \App\Models\Product::where('stock', '>', 0)
                            ...
                            ->get();
                    @endphp
                    @forelse($rekomendasiPerluDibeli as $product)
                        <div
                            class="flex flex-col overflow-hidden transition bg-white/90 border border-green-200 rounded-2xl shadow-lg hover:shadow-2xl h-full backdrop-blur-lg hover:-translate-y-1 hover:scale-[1.02] duration-300">
                            <img src="{{ asset($product->image_url) }}" alt="{{ $product->name }}"
                                class="object-cover w-full h-40 border-b border-green-100 rounded-t-2xl" loading="lazy">
                            <div class="flex flex-col flex-1 p-6">
                                ...
                                <div class="flex flex-row gap-2 mt-auto">
                                    ...
                                    @else
                                        <!-- And change it to use direct URL instead: -->
                ...
                <div class="p-6 mb-12 shadow-lg rounded-2xl bg-gradient-to-br from-white/90 to-white/80 backdrop-blur-md">
                    <div class="flex flex-col items-center gap-8 lg:flex-row">
                        <div class="w-full lg:w-1/2">
                            <h2 class="mb-4 text-2xl font-bold text-green-800 md:text-3xl">Kunjungi Kami di Marketplace
                            </h2>
                            <p class="mb-6 leading-relaxed text-gray-700">
                                Dapatkan kemudahan berbelanja tanaman dari Azka Garden melalui platform marketplace
                                terpercaya. Kami menawarkan koleksi tanaman berkualitas, perlengkapan berkebun, dan layanan
                            ...
                            <div class="flex flex-col gap-4 mb-6 sm:flex-row">
                                ...
                                <a href="https://www.tokopedia.com/hendrikfloris" target="_blank" rel="noopener"
                                    ...
                                    <div
                                        class="transition-all duration-300 transform translate-x-2 opacity-0 group-hover:opacity-100 group-hover:translate-x-0">
                            ...
                            <div class="p-4 border border-green-100 rounded-lg bg-green-50">
                                <div class="flex items-start gap-3">
                                    <svg class="w-5 h-5 mt-0.5 text-green-600 flex-shrink-0" fill="none"
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-700">Buka 24/7 untuk layanan online. Pengiriman ke
                                            seluruh Indonesia. Konsultasi gratis untuk pemilihan tanaman terbaik sesuai
                                            kebutuhan Anda.</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex justify-center w-full lg:w-1/2">
                            <div class="relative p-1 overflow-hidden bg-white shadow-xl rounded-2xl">
                                <div
                                    class="absolute inset-0 bg-gradient-to-br from-green-500/10 to-green-700/30 rounded-2xl">
                                </div>
                                <img src="{{ asset('images/tokopedia-showcase.jpg') }}"
                                    alt="Toko Bunga Hendrik di Tokopedia"
                                    class="relative z-10 w-full h-auto max-w-md rounded-xl" loading="lazy"
                                    onerror="this.onerror=null;this.src='{{ asset('images/placeholder-store.jpg') }}';">
                                <div
                                    class="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/70 to-transparent">
                                    <div class="flex items-center gap-3">
                                        <img src="{{ asset('images/tokopedia-icon.png') }}" alt="Tokopedia"
                                            class="w-8 h-8">
        ...
        <div class="absolute left-0 right-0 z-40 flex flex-col items-center select-none" style="bottom: 10px;">
            <div class="flex items-center justify-center gap-3 px-4 py-2 rounded-full shadow-lg bg-black/30">
                <button
                    ...
                    @mouseenter="stopInterval" @mouseleave="startInterval" @click="prevBackground"
                    aria-label="Background Sebelumnya" style="user-select:none;">
                    &#8592;
                </button>
                <div class="flex justify-center gap-3">
                    <template x-for="(img, idx) in heroImages" :key="'herodot' + idx">
                        <span
                            :class="{
                                'w-5 h-3 rounded-full bg-green-500 shadow-lg cursor-pointer ring-2 ring-green-400 ring-offset-2': idx ===
                                    currentBg,
                                'w-3 h-3 rounded-full bg-white/60 cursor-pointer': idx !== currentBg
                            }"
                            @click="setBg(idx)"
                            style="user-select:none;transition:all 0.4s cubic-bezier(.4,0,.2,1);"></span>
                    </template>
                </div>
                <button
                    class="flex items-center justify-center w-10 h-10 text-lg text-white transition rounded-full shadow bg-black/50 hover:bg-black/70"
                    @mouseenter="stopInterval" @mouseleave="startInterval" @click="nextBackground"
                    aria-label="Background Selanjutnya" style="user-select:none;">
                    &#8594;
                </button>
            </div>
        </div>
