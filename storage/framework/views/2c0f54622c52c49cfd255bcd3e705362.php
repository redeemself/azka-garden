...
<div x-data="{ loading: true }" x-init="setTimeout(() => loading = false, 400)">
  ...
  <div class="px-2 mx-auto sm:px-5 max-w-7xl" x-show="!loading" x-transition.opacity>
        ...
        <table class="orders-table">
          ...
          <tbody>
            ...
            <tr class="waiting-row">
              <td data-label="Kode Order" class="column-order-code"><span class="font-mono font-bold" style="color:#295f3a;"><?php echo e($order->order_code); ?></span></td>
              <td data-label="Tanggal" class="column-date">
                <span class="block"><?php echo e(\Carbon\Carbon::parse($order->order_date ?? $order->created_at)->format('d M Y')); ?></span>
                <span class="block text-xs" style="color:#888e92;"><?php echo e(\Carbon\Carbon::parse($order->order_date ?? $order->created_at)->format('H:i')); ?></span>
              </td>
              ...
              <td data-label="Total Harga" class="column-price">
                <span class="total-price">Rp<?php echo e(number_format($totalWithTax, 0, ',', '.')); ?></span>
              </td>
              ...
              <td data-label="Pengiriman" class="column-shipping">
                ...
                <div class="shipping-info">
                  <span class="font-semibold"><?php echo e($order->shipping->courier ?? $shippingMethod); ?></span>
                  ...
                  <span class="block" style="color:#5f8b6a;">
                    ...
                    <?php else: ?>
                    Rp<?php echo e(number_format($order->shipping->shipping_cost ?? $shippingCost, 0, ',', '.')); ?>
                ...
                <?php else: ?>
                <div class="shipping-info">
                  <span class="font-semibold"><?php echo e($shippingMethod); ?></span>
                  <?php if($isFreeShipping): ?>
                  <span class="free-shipping-badge">BEBAS ONGKIR</span>
                  <?php endif; ?>
                  <span class="block" style="color:#5f8b6a;">
              ...
              <td data-label="Pembayaran" class="column-payment">
                <div class="payment-info">
                  <span class="payment-label">Metode: <?php echo e($order->payment->method->name ?? $paymentMethod); ?></span><br>
                  <span class="payment-label">Status: Menunggu Pembayaran</span><br>
                  <span style="font-size:0.97em;">Total:</span>
                  <span class="payment-nominal">Rp<?php echo e(number_format($totalWithTax, 0, ',', '.')); ?></span>
                </div>
              </td>
              <td data-label="Aksi" class="column-action">
                <div class="action-btns">
                  <a href="<?php echo e(route('user.orders.show', $order->id)); ?>" class="action-btn btn-detail">Detail Pesanan</a>
                  <button type="button" class="action-btn btn-cancel" onclick="document.getElementById('cancel-form-<?php echo e($order->id); ?>').submit()">Batalkan</button>
                  <button type="button" class="action-btn btn-expire" onclick="showExpireConfirm(<?php echo e($order->id); ?>)">Jadikan Kadaluarsa</button>
                  <button type="button" class="action-btn btn-complete" onclick="document.getElementById('complete-form-<?php echo e($order->id); ?>').submit()">Selesaikan</button>
                
                  <form id="cancel-form-<?php echo e($order->id); ?>" action="<?php echo e(route('user.orders.cancel', $order->id)); ?>" method="POST" style="display:none;">
                    <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                  </form>
                  <form id="expire-form-<?php echo e($order->id); ?>" action="<?php echo e(route('user.orders.expire', $order->id)); ?>" method="POST" style="display:none;" class="expire-form" data-order-id="<?php echo e($order->id); ?>">
                    <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                  </form>
                  <form id="complete-form-<?php echo e($order->id); ?>" action="<?php echo e(route('user.orders.complete', $order->id)); ?>" method="POST" style="display:none;">
                    <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                  </form>
                </div>
              </td>
    ...
    <?php if($processingOrShippedOrders->count()): ?>
    ...
    <div class="orders-table-container">
      ...
      <div class="overflow-x-auto">
        <table class="orders-table">
          ...
          <tbody>
            <?php $__currentLoopData = $processingOrShippedOrders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
              $statusVal = $getStatusValue($order->status);
              $totalWithTax = $getTotalWithTax($order);
            ...
            ?>
            <tr>
              <td data-label="Kode Order" class="column-order-code"><span class="font-mono font-bold" style="color:#888;"><?php echo e($order->order_code); ?></span></td>
              <td data-label="Tanggal" class="column-date">
                <span class="block"><?php echo e(\Carbon\Carbon::parse($order->order_date ?? $order->created_at)->format('d M Y')); ?></span>
                <span class="block text-xs" style="color:#888e92;"><?php echo e(\Carbon\Carbon::parse($order->order_date ?? $order->created_at)->format('H:i')); ?></span>
              </td>
              <td data-label="Status" class="column-status">
                <div class="status-cell-wrapper">
                  <span class="status-badge status-processing"><?php echo e($statusText); ?></span>
                </div>
              </td>
              <td data-label="Total Harga" class="column-price">
                <span class="total-price" style="color:#888;">Rp<?php echo e(number_format($totalWithTax, 0, ',', '.')); ?></span>
              </td>
              <td data-label="Produk" class="column-product">
                <div class="product-list">
                  <?php $__currentLoopData = $order->details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $detail): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              ...
              <td data-label="Pengiriman" class="column-shipping">
                ...
                <div class="shipping-info">
                  ...
                  <span class="block">Resi: <span class="font-mono"><?php echo e($order->shipping->tracking_number ?? '-'); ?></span></span>
                  <span class="block" style="color:#888;">Status: <?php echo e($order->shipping->status ?? $statusText); ?></span>
                </div>
                <?php else: ?>
                <div class="shipping-info">
                  <span class="font-semibold"><?php echo e($shippingMethod); ?></span>
                  ...
                  <span class="block">Status: <?php echo e($statusText); ?></span>
                </div>
                <?php endif; ?>
              </td>
              <td data-label="Pembayaran" class="column-payment">
                <div class="payment-info">
                  <span class="payment-label">Metode: <?php echo e($order->payment->method->name ?? $paymentMethod); ?></span><br>
                  <span class="payment-label">Status: <?php echo e($statusText); ?></span><br>
                  <span style="font-size:0.97em;">Total:</span>
                  <span class="payment-nominal">Rp<?php echo e(number_format($totalWithTax, 0, ',', '.')); ?></span>
                </div>
              </td>
        ...
        <table class="orders-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td data-label="Status" class="column-status">
                <div class="status-cell-wrapper">
                  <span class="status-badge status-completed"><?php echo e($statusText); ?></span>
                </div>
              </td>
              <td data-label="Total Harga" class="column-price">
                <span class="total-price">Rp<?php echo e(number_format($totalWithTax, 0, ',', '.')); ?></span>
              ...
              <td data-label="Pengiriman" class="column-shipping">
                ...
                <div class="shipping-info">
                  ...
                  <span class="block">Resi: <span class="font-mono"><?php echo e($order->shipping->tracking_number ?? '-'); ?></span></span>
                  <span class="block" style="color:#5f8b6a;">Status: <?php echo e($order->shipping->status ?? 'Selesai'); ?></span>
                </div>
                <?php else: ?>
                <div class="shipping-info">
                  <span class="font-semibold"><?php echo e($shippingMethod); ?></span>
    ...
    <div class="canceled-warning">
      ...
      <span>
        ...
        <a href="<?php echo e(route('user.orders.history.index')); ?>">Halaman Riwayat</a>.
      </span>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="canceled-orders-table">
          ...
          <tbody>
            ...
            ?>
            <tr>
              <td data-label="Kode Order" class="column-order-code"><span class="font-mono font-bold" style="color:#888e92;"><?php echo e($order->order_code); ?></span></td>
              <td data-label="Tanggal" class="column-date">
                <span class="block"><?php echo e(\Carbon\Carbon::parse($order->order_date ?? $order->created_at)->format('d M Y')); ?></span>
                <span class="block text-xs" style="color: #888e92;"><?php echo e(\Carbon\Carbon::parse($order->order_date ?? $order->created_at)->format('H:i')); ?></span>
              </td>
              <td data-label="Status" class="column-status">
                <div class="status-cell-wrapper">
                  <span class="status-badge status-canceled"><?php echo e($statusText); ?></span>
                </div>
              </td>
              <td data-label="Total Harga" class="column-price">
                <span class="total-price price-strikethrough">Rp<?php echo e(number_format($totalWithTax, 0, ',', '.')); ?></span>
              </td>
              <td data-label="Produk" class="column-product">
                <div class="product-list">
                  <?php $__currentLoopData = $order->details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $detail): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  ...
                  <div class="product-item">
                    <img src="<?php echo e($img); ?>" alt="<?php echo e($detail->product->name); ?>" class="product-img" />
                    <div class="product-info">
                      <span class="product-title" style="color:#888e92;"><?php echo e($detail->product->name); ?></span>
                      <span class="product-qty">x<?php echo e($detail->quantity); ?></span>
              ...
              <td data-label="Pengiriman" class="column-shipping">
                <?php if($order->shipping): ?>
                <div class="shipping-info">
                  <span class="font-semibold" style="color:#888e92;"><?php echo e($order->shipping->courier ?? $shippingMethod); ?></span>
                  <?php if($isFreeShipping): ?>
                  ...
                  <span class="block" style="color:#888e92;">
                    ...
                    <?php else: ?>
                    <span class="price-strikethrough">Rp<?php echo e(number_format($order->shipping->shipping_cost ?? $shippingCost, 0, ',', '.')); ?></span>
                    <?php endif; ?>
                  </span>
                  <span class="block">Resi: <span class="font-mono"><?php echo e($order->shipping->tracking_number ?? '-'); ?></span></span>
                  <span class="block" style="color:#888e92;">Status: <?php echo e($order->shipping->status ?? 'Dibatalkan'); ?></span>
                </div>
                <?php else: ?>
                <div class="shipping-info">
                  <span class="font-semibold" style="color:#888e92;"><?php echo e($shippingMethod); ?></span>
                  <?php if($isFreeShipping): ?>
                  <span class="free-shipping-badge" style="background:#888e92;">BEBAS ONGKIR</span>
                  <?php endif; ?>
                  <span class="block" style="color:#888e92;">
                    ...
                    <span class="price-strikethrough">Gratis (Rp0)</span>
                    <?php else: ?>
                    <span class="price-strikethrough">Rp<?php echo e(number_format($shippingCost, 0, ',', '.')); ?></span>
                    <?php endif; ?>
                  </span>
              ...
              <td data-label="Pembayaran" class="column-payment">
                <div class="payment-info" style="background:#f8faf8;">
                  ...
                  <span class="payment-nominal price-strikethrough">
                    Rp<?php echo e(number_format($order->payment->total ?? $totalWithTax, 0, ',', '.')); ?>
    ...
    <div class="expired-warning">
      ...
      <div class="overflow-x-auto">
        <table class="expired-orders-table">
          ...
          <tbody>
            <?php $__currentLoopData = $expiredOrders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
              $totalWithTax = $getTotalWithTax($order);
              $shippingMethod = $getShippingMethodName($order->shipping_method ?? 'KURIR_TOKO');
              $paymentMethod = $getPaymentMethodName($order->payment_method ?? 'CASH');
            ...
            <tr>
              ...
              <td data-label="Pengiriman" class="column-shipping">
                ...
                <div class="shipping-info">
                  ...
                  <span class="block">Resi: <span class="font-mono"><?php echo e($order->shipping->tracking_number ?? '-'); ?></span></span>
                  <span class="block" style="color:#888e92;">Status: <?php echo e($order->shipping->status ?? 'Kadaluarsa'); ?></span>
                </div>
                <?php else: ?>
                <div class="shipping-info">
                  <span class="font-semibold" style="color:#888e92;"><?php echo e($shippingMethod); ?></span>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\azka-garden\resources\views/user/orders/index.blade.php ENDPATH**/ ?>